...
impl Display for MenuErr {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        return match &self {
            MenuErr::IO(error, line, file) => {
                write!(f, "Internal IO Error at {line} in {file}: {}", error)
            }
        };
    }
}
...
pub fn print_menu<'a, Q: Into<ListItem<'a>> + Clone>(
    prompt: &'a str,
    choices: Vec<Q>,
) -> Result<Option<Q>, MenuErr> {
    ...
    match menu_result {
        Ok(_) => {
            return Ok(Some(
                new_select.choices[new_select.final_choice_index].clone(),
            ));
        }
        Err(error) => {
...
impl<'a, Q: Into<ListItem<'a>> + Clone> Widget for &mut SelectionScreen<'a, Q> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let list = List::new(self.choices.clone())
            .block(block)
            .highlight_symbol(">");
